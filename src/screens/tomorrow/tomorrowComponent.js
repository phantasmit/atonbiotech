...
const TomorrowComponent = () => {
    ...
    const tomorrowRecords = appointmentData.filter(
        (record) => getDatePart(record.appointment_at) === tomorrow
    );


    const filtered = useMemo(() => {
        if (!search.trim()) return tomorrowRecords;
        const q = search.trim().toLowerCase();
        return tomorrowRecords.filter(
            (a) => a.doctor_name.toLowerCase().includes(q) || a.status.toLowerCase().includes(q)
        );
    }, [search, tomorrowRecords]);
                ...
                ListHeaderComponent={
                    <>
                        <View style={[styles.subHeader, !isTableLayout && styles.subHeaderStacked]}>
                            <Text style={styles.titleText}>Tomorrow Appointments</Text>
...
export default TomorrowComponent;
