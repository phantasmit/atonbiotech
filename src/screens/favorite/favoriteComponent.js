...
const FavoriteComponent = () => {
    ...
    //
    const onRefresh = useCallback(() => fetchProducts('refresh'), [fetchProducts]);
    const onRetry = useCallback(() => fetchProducts('initial'), [fetchProducts]);
    ...
    const displayedItems = useMemo(() => {
        ...
        list = [...list].sort((a, b) =>
            sortAsc ? (a?.product.name || '').localeCompare(b?.product.name || '') : (b?.product.name || '').localeCompare(a?.product.name || '')
        );
        return list;
        ...
        return (
            ...
            >
                ...
                <View style={styles.cardContent}>
                    ...
                    {!!packaging && (
                        <Text style={styles.itemQty} numberOfLines={1}>{packaging}</Text>
                    )}
        ...
        return (
            ...
            >
                ...
                <View style={styles.gridContent}>
                    <Text style={styles.itemName} numberOfLines={2}>{name}</Text>
                    <Text style={styles.itemMrp} numberOfLines={1}>MRP {formatCurrency(mrp)}</Text>
                </View>
            </TouchableOpacity>
        );
    };
