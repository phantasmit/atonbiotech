...
import { CREATE_LABEL_API, UPDATE_DOCTOR_NAME_API, UPDATE_LABEL_API } from '../services/api-end-points';
...
const EditDoctorModal = ({ route }) => {
    ...
    const handleDone = async () => {
        ...
        try {
            if (indexPosition === 1) {
                await request(UPDATE_DOCTOR_NAME_API(id), HTTP_METHODS.PUT, JSON.stringify({
                    "name": labelName,
                }))
                dispatch(fetchHospitals())
            } else {
                await request(UPDATE_LABEL_API(id), HTTP_METHODS.PUT, JSON.stringify({
                    "name": labelName,
                }))
            ...
            >
                ...
                <View style={[styles.card, cardStyle]}>
                    ...
                    <View style={[styles.header, { paddingTop: Math.max(18, insets.top > 0 ? 18 : 18) }]}>
                        <Text style={[styles.headerTitle, { fontSize: fontScale(18) }]}>{(indexPosition === 1) ? `Update Doctor` : `Update Label`}</Text>
                    ...
                    >
                        ...
                        <Text style={[styles.label, { fontSize: fontScale(13) }]}>{(indexPosition === 1) ? 'Doctor Name' : `Label Name`}</Text>
                        <TextInput
                            style={styles.input}
                            value={labelName}
